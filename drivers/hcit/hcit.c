...
void hcit_gpioled_ctl(unsigned long arg)
{
	...
	if (copy_from_user
	    (&led, (led_rpc_cmd_type *) arg, sizeof(led_rpc_cmd_type))) {
		printk("[HCIT] hcit_gpioled_ctl() copy_from_user failure \r\n");
		return;
	}
	printk
	    ("[HCIT]hcit_gpioled_ctl() , index<%d> mode<%d> period<%ld> ontime<%ld>\r\n",
	     led.index, led.mode, led.period, led.ontime);

	printk
	    ("[HCIT]hcit_gpioled_ctl() , sizeof(ledcmd)<%d> sizeof(ul)<%d> sizeof(int)<%d> sizeof(char)<%d>\r\n",
	     sizeof(led_rpc_cmd_type), sizeof(unsigned long), sizeof(int),
	     sizeof(char));

	// Add rpc interface here
	oem_rpc_client_register(RPC_CHANNEL_LED);
	set_data_to_arm9(RPC_CHANNEL_LED, (char *)&led,
			 sizeof(led_rpc_cmd_type));

}

static ssize_t hcit_read(struct file *file, char __user * buf, size_t count,
			 loff_t * ppos)
{
...
static long hcit_ioctl(struct file *filp, unsigned int cmd,
		      unsigned long arg)
{
	int err = 0;
