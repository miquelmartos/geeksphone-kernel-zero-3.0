...
static long ecs_ctrl_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ECOMPASS_IOC_SET_YPR:
		...
		/* Report ambient light sensor information */
		if (atomic_read(&l_flag)) {
			input_report_abs(ecs_data_device, ABS_VOLUME, ypr[14]);
			input_report_abs(ecs_data_device, ABS_MISC, ypr[15]);
		}
	...
	//pr_info("ecompass ecs_ctrl_ioctl--\n");
