...
{
	__raw_writel(__raw_readl(reg) | bit(n), reg);
}
...
static int gpio_chip_get(struct gpio_chip *chip, unsigned offset)
{
	...
	spin_lock_irqsave(&msm_gpio->lock, irq_flags);
	ret = __raw_readl(msm_gpio->regs.in) & bit(offset) ? 1 : 0;
	spin_unlock_irqrestore(&msm_gpio->lock, irq_flags);
...
static inline void forget_level_irq(struct msm_gpio_dev *msm_gpio,
				unsigned offset)
{
	unsigned v = __raw_readl(msm_gpio->regs.int_edge);
	unsigned b = bit(offset);

	if (!(v & b))
		__raw_writel(b, msm_gpio->regs.int_clear);
...
{
	unsigned long irq_flags;
	struct msm_gpio_dev *msm_gpio = irq_data_get_irq_chip_data(d);
	unsigned offset = d->irq - msm_gpio->irq_base;

	spin_lock_irqsave(&msm_gpio->lock, irq_flags);
	forget_level_irq(msm_gpio, offset);
	msm_gpio->irq_masks[IRQ_MASK_NORMAL] &= ~bit(offset);
	__raw_writel(msm_gpio->irq_masks[IRQ_MASK_NORMAL], msm_gpio->regs.int_en);
	spin_unlock_irqrestore(&msm_gpio->lock, irq_flags);
}
...
/*
static int msm_gpio_irq_set_affinity(struct irq_data *d, const struct cpumask *dest, bool force)
{
	return -ENOTSUPP;
}

static int msm_gpio_irq_retrigger(struct irq_data *d)
{
	return -ENOTSUPP;
}
...
static int msm_gpio_irq_set_wake(struct irq_data *d, unsigned int on)
{
	...
	return irq_set_irq_wake(msm_gpio->irq_summary, on);
}
...
static irqreturn_t msm_gpio_irq_handler(int irq, void *dev)
{
	 ...
	 */
	spin_lock_irqsave(&msm_gpio->lock, irq_flags);
	s = __raw_readl(msm_gpio->regs.int_status);
	e = __raw_readl(msm_gpio->regs.int_en);
	v = s & e;
	if (v)
		__raw_writel(v, msm_gpio->regs.int_clear);
	spin_unlock_irqrestore(&msm_gpio->lock, irq_flags);
...
//	.irq_retrigger		= msm_gpio_irq_retrigger,
...
static int __devinit msm_gpio_probe(struct platform_device *dev)
{
	...
	for (i = 0; i < msm_gpio->gpio_chip.ngpio; ++i) {
		...
		irq_set_chip_data(irq, msm_gpio);
...
static int msm_gpio_suspend(struct msm_gpio_dev *msm_gpio)
{
	...
	if ((msm_gpio->nsuspend)++ == 0) {
		...
		__raw_writel(irq_mask, msm_gpio->regs.int_en);
	}
	spin_unlock_irqrestore(&msm_gpio->lock, irq_flags);
...
static int msm_gpio_resume(struct msm_gpio_dev *msm_gpio)
{
	...
	spin_lock_irqsave(&msm_gpio->lock, irq_flags);
	if (--(msm_gpio->nsuspend) == 0)
		__raw_writel(msm_gpio->irq_masks[IRQ_MASK_NORMAL],
